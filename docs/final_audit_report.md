...
- attempted `docker compose up -d --build`
...
- Docker runtime startup could not be verified because the local Docker daemon was unavailable in this environment.
...
- Docker container startup could not be fully validated here because the local Docker daemon was not running.
